...
public abstract class Critter {
   ...
   // public abstract CritterShape viewShape();
    ...
    private	static List<Critter> populationCopy = new java.util.ArrayList<Critter>();
	...
	private int x_coord;
	private int y_coord;
	private boolean getHasLookedinDoTimeStep(){
	    return false;
    }
    /**
    ...
    public static void makeCritter(String critter_class_name) throws InvalidCritterException {
        try{
            String updatedClass = "Assignment5." + critter_class_name;
            ...
            newCrit.x_coord = getRandomInt(Painter.numRows);
            newCrit.y_coord = getRandomInt(Painter.numCols);
    ...
    protected final String look(int direction, boolean steps)
    {
        ...
        if(!getHasLookedinDoTimeStep()) {
            ...
            if (!steps) {
                ...
                moveHelper(1, direction);                    //temporarily moves the critter to the desired spot
                if (alreadyOccupiedOld(x_coord, y_coord))                //checks to see if the spot is already occupied
                {
                    tempX2 = x_coord;
                    tempY2 = y_coord;
                    x_coord = tempX;
                    y_coord = tempY;
                    occupied = checkOccupancyOld(tempX2, tempY2);     //to-string of the occupied critter
                }
                x_coord = tempX;
                y_coord = tempY;
                energy -= look_energy_cost;
        ...
        //use java class Method to access static sublclass runStats function
            ...
            try {
                if (method != null && (method.getDeclaringClass() != Critter.class)) {
                    ...
                    stats = (String) method.invoke(critters.get(0).getClass(), critters);
                    return stats;
                }
    ...
    /**
     * Determines if another critter is occupying the same spot, but compares to the old positons
     * @param x_coord
     * @param y_coord
     * @return nothing
     */
    private static boolean alreadyOccupiedOld(int x_coord, int y_coord)
    {
        int same = 0;
        for(int i = 0;i<populationCopy.size();i++) {
            if(populationCopy.get(i).x_coord == x_coord && populationCopy.get(i).y_coord == y_coord) {
                same++;
            }
        }
        if(same > 1) {
	...
	public static void worldTimeStep() {
       ...
       while(iter.hasNext()) {
        ...
        for (int i = 0; i < refresh_algae_count; i++) {
            ...
            addAlg.setX_coord(getRandomInt(Painter.numRows));
            addAlg.setY_coord(getRandomInt(Painter.numCols));
     ...
     * If there is a critter occupying a point on the grid it will return it's String representation to be printed.
     * If not, will return return a blank space.
     * Used in the special case of look being called in doTimeStep
     * @param x
     * @param y
     * @return String representation of a critter if spot is occupied, else a space
     */
    public static String checkOccupancyOld(int x, int y){
        for(int i = 0; i < populationCopy.size(); i++){
            if(populationCopy.get(i).x_coord == x && populationCopy.get(i).y_coord == y){
                return populationCopy.get(i).toString();
            }
        }
        return " ";
    }
	...
	public static void displayWorld(GridPane gridPane) {
        NumberBinding shapeBind = Bindings.min(gridPane.heightProperty(),gridPane.widthProperty());
	    grid.getChildren().retainAll(grid.getChildren().get(0));
	    Circle temp = null;
        for(int i =0;i<population.size();i++)
        {
            temp = new Circle(Painter.size/4);
            Color c = population.get(i).viewFillColor();
            temp.setFill(c);
            temp.radiusProperty().bind(shapeBind.divide(Painter.numCols*2));
            gridPane.add(temp,population.get(i).x_coord,population.get(i).y_coord);
            GridPane.setHalignment(temp, HPos.CENTER);
        }
	}

	public static void makeGrid(GridPane gridPane){
        int numRows = world_width / 12;
        int numCols = world_height / 12;
        int size = world_width / numRows;
